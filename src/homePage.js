export function loadHomePage() {
    ...
    listDiv.style.display = 'inline-block'; // Show home page list
    listDiv.innerHTML = ``;

    const homePageDetails = document.getElementById("homePage");
    homePageDetails.style.display = 'block'; // Ensure home page content is visible
    homePageDetails.innerHTML = `
        <div>
            <h1 class="title-home">Today</h1>
        </div>
        <div id="task-amount-home">No tasks</div>
    `;
    ...
    function listLogic() {
        function updateTaskAmount() {
            ...
            const taskAmount = document.getElementById("task-amount-home");
            taskAmount.innerHTML = `${listLength} tasks`;
        }

        function displayItem() {
            console.log("Displaying items. Current myList:", myList);
            listDiv.innerHTML = ''; 
            updateTaskAmount();
            ...
            myList.forEach(function(item) {
                ...
                deleteButton.addEventListener("click", function(event) {
                    ...
                    myList = myList.filter(i => i.id !== item.id); // Remove item from list
        ...
        function setupFormListener() {
            ...
            newFormElement.addEventListener("submit", function(e) {
                ...
                if (currentEditId === null) {
                    ...
                    const item = myList.find(item => item.id === currentEditId);
                    if (item) {
                        item.title = title;
    ...
    function openEditPopup(itemId) {
        const item = myList.find(item => item.id === itemId);
        if (item) {
            ...
            saveBtn.addEventListener("click", function() {
                const editedItem = myList.find(item => item.id === currentEditId);
                if (editedItem) {
